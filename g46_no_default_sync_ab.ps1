# G46 GPU-resident route no-default-sync A/B (PowerShell 5.1, ASCII).
param([switch]$Resume)
...
function Invoke-G46Run {
    ...
    if ($Arm -eq "no-default-sync") {
        $args += "-RouteNoDefaultSync"
    }

    if ($Resume -and (Test-Path -LiteralPath $resultPath -PathType Leaf)) {
        Write-Host ("[g46] resume tag=" + $Tag + " arm=" + $Arm)
    } else {
        Write-Host ("[g46] start tag=" + $Tag + " arm=" + $Arm)
        & powershell.exe @args | ForEach-Object { Write-Host $_ }
        if ($LASTEXITCODE -ne 0) { throw "G46 run failed: $Tag" }
    }

    if (-not (Test-Path -LiteralPath $resultPath -PathType Leaf)) {
        throw "G46 result missing: tag=$Tag"
    }
    ...
    if (-not $result.outputs_identical -or
        ...
        throw "G46 contract mismatch: tag=$Tag"
    }
    if ($noSyncExpected) {
        if ($result.gpu_resident_routes_default_sync_calls -ne 0 -or
            $result.gpu_resident_routes_no_default_sync_calls -ne
                $result.gpu_resident_routes_calls) {
            throw "G46 no-sync accounting mismatch: tag=$Tag"
        }
        ...
        throw "G46 default-sync accounting mismatch: tag=$Tag"
    }

    $ttft = [double]$result.server_prefill_ttft_mean_seconds
    $wrap = [double]$result.prefill_mass_wrap_seconds
    $resolve = [double]$result.gpu_resident_routes_resolve_ms_per_call
    $wait = [double]$result.gpu_resident_routes_wait_ms_per_call
    [pscustomobject]@{
        tag = $Tag
        arm = $Arm
        ...
        route_resolve_ms_per_call = $resolve
        route_wait_ms_per_call = $wait
        route_handoff_ms_per_call = [math]::Round($resolve + $wait, 6)
...
$runs = @()
$runs += Invoke-G46Run -Tag "g46_default_a" -Arm "default-sync"
$runs += Invoke-G46Run -Tag "g46_nosync_a" -Arm "no-default-sync"
$runs += Invoke-G46Run -Tag "g46_nosync_b" -Arm "no-default-sync"
$runs += Invoke-G46Run -Tag "g46_default_b" -Arm "default-sync"
$runs += Invoke-G46Run -Tag "g46_default_c" -Arm "default-sync"
$runs += Invoke-G46Run -Tag "g46_nosync_c" -Arm "no-default-sync"
...
foreach ($field in $provenanceFields) {
    ...
    if ($values.Count -ne 1) { throw "G46 mixed provenance: field=$field" }
}

$armSummary = @()
foreach ($arm in @("default-sync", "no-default-sync")) {
    $rows = @($runs | Where-Object { $_.arm -eq $arm })
    if ($rows.Count -ne 3) { throw "G46 replication mismatch: arm=$arm" }
    $armSummary += [pscustomobject]@{
        arm = $arm
        independent_processes = $rows.Count
        decode_tokens_per_second_mean = Get-G46Mean $rows "decode_tokens_per_second"
        decode_tokens_per_second_median = Get-G46Median $rows "decode_tokens_per_second"
        decode_seconds_mean = Get-G46Mean $rows "decode_seconds"
        decode_seconds_median = Get-G46Median $rows "decode_seconds"
        ttft_mean_seconds = Get-G46Mean $rows "ttft_seconds"
        ttft_median_seconds = Get-G46Median $rows "ttft_seconds"
        wrap_mean_seconds = Get-G46Mean $rows "wrap_seconds"
        wrap_median_seconds = Get-G46Median $rows "wrap_seconds"
        ttft_minus_wrap_mean_seconds = Get-G46Mean $rows "ttft_minus_wrap_seconds"
        vram_hits_mean = Get-G46Mean $rows "vram_hits"
        ram_hits_mean = Get-G46Mean $rows "ram_hits"
        ram_h2d_gib_mean = Get-G46Mean $rows "ram_h2d_gib"
        route_worker_ms_per_job_mean = Get-G46Mean $rows "route_worker_ms_per_job"
        route_resolve_ms_per_call_mean = Get-G46Mean $rows "route_resolve_ms_per_call"
        route_wait_ms_per_call_mean = Get-G46Mean $rows "route_wait_ms_per_call"
        route_handoff_ms_per_call_mean = Get-G46Mean $rows "route_handoff_ms_per_call"
        vram_peak_mib_mean = Get-G46Mean $rows "vram_peak_mib"
...
$summary = [pscustomobject]@{
    schema = "g46_no_default_sync_ab_v1"
    question = "Can the GPU-resident route handoff rely on mapped request/worker-ready publication without an explicit default-stream synchronization and improve exact 64-token decode?"
    prompt = $prompt
    context = 256
    max_tokens = 64
    cache_experts = 320
...
$summaryPath = Join-Path $outdir "g46_no_default_sync_ab_result.json"
$summary | ConvertTo-Json -Depth 10 | Set-Content -Encoding UTF8 $summaryPath
Write-Host ("[g46] matrix complete: " + $summaryPath)
