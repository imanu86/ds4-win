...
function Invoke-G51Run {
    param(
        [Parameter(Mandatory=$true)][string]$Tag,
        [Parameter(Mandatory=$true)][ValidateSet("control", "prefill-vram-seed")][string]$Arm,
        [Parameter(Mandatory=$true)][int]$PrefillVramSeedPerLayer
    )
    ...
    if ($Arm -eq "prefill-vram-seed") {
        $args += @("-PrefillVramSeedPerLayer", ([string]$PrefillVramSeedPerLayer))
    }

    if (($Resume -or $SummarizeExisting) -and
        (Test-Path -LiteralPath $resultPath -PathType Leaf)) {
        Write-Host ("[g51] validate existing tag=" + $Tag + " arm=" + $Arm)
    ...
    $decodeSeconds = [double]$r.server_runs[0].server_decode_seconds
    ...
    [pscustomobject]@{
        ...
        route_pinned_ram_routes = [uint64]$r.gpu_resident_routes_pinned_ram_routes
        route_h2d_gib = Convert-G51BytesToGiB $r.gpu_resident_routes_h2d_bytes
        route_worker_jobs = [uint64]$r.gpu_resident_routes_worker_jobs
        route_miss_experts = [uint64]$r.gpu_resident_routes_miss_experts
        route_worker_ms_per_job = [double]$r.gpu_resident_routes_worker_ms_per_job
        route_wait_ms_per_call = [double]$r.gpu_resident_routes_wait_ms_per_call
        route_resolve_ms_per_call = [double]$r.gpu_resident_routes_resolve_ms_per_call
        prefill_vram_seed_requested_per_layer = [int]$r.prefill_vram_seed_requested_per_layer
        ...
        prefill_vram_seed_gib = Convert-G51BytesToGiB $r.prefill_vram_seed_bytes
        ...
        runtime_process_read_gib = Convert-G51BytesToGiB $rt.win32_process_read_transfer_delta_bytes
        runtime_process_write_gib = Convert-G51BytesToGiB $rt.win32_process_write_transfer_delta_bytes
        runtime_vram_peak_mib = [uint64]$rt.vram_used_peak_mib
        runtime_dedicated_peak_gib = Convert-G51BytesToGiB $rt.gpu_process_dedicated_peak_bytes
        runtime_shared_peak_gib = Convert-G51BytesToGiB $rt.gpu_process_shared_peak_bytes
        runtime_working_set_peak_gib = Convert-G51BytesToGiB $rt.process_working_set_peak_bytes
        runtime_page_fault_delta = [uint64]$rt.page_fault_delta
        runtime_available_min_gib = Convert-G51BytesToGiB $rt.windows_available_min_bytes
...
if (-not $SummarizeExisting -and
    -not (Test-G51HarnessParameter -ParameterName "PrefillVramSeedPerLayer")) {
    throw "Harness does not expose -PrefillVramSeedPerLayer; refusing to launch model."
}

$runs = @()
foreach ($suffix in @("a", "b", "c")) {
    $runs += Invoke-G51Run -Tag ("g51_prefill_vram_seed_off_" + $suffix) `
        -Arm "control" -PrefillVramSeedPerLayer 0
    $runs += Invoke-G51Run -Tag ("g51_prefill_vram_seed_on_" + $suffix) `
        -Arm "prefill-vram-seed" -PrefillVramSeedPerLayer 8
}
...
foreach ($field in $provenanceFields) {
    ...
    if ($values.Count -ne 1) {
        throw "G51 mixed provenance across runs: field=$field"
    }
}
...
foreach ($arm in @("control", "prefill-vram-seed")) {
    ...
    if ($rows.Count -ne 3) { throw "G51 replication mismatch: arm=$arm" }
    $armSummary += [pscustomobject]@{
        arm = $arm
        independent_processes = $rows.Count
        decode_tokens_per_second_mean = Get-G51Mean $rows "decode_tokens_per_second"
        decode_tokens_per_second_median = Get-G51Median $rows "decode_tokens_per_second"
        ...
        route_pinned_ram_routes_mean = Get-G51Mean $rows "route_pinned_ram_routes"
        route_h2d_gib_mean = Get-G51Mean $rows "route_h2d_gib"
        route_worker_jobs_mean = Get-G51Mean $rows "route_worker_jobs"
        route_wait_ms_per_call_mean = Get-G51Mean $rows "route_wait_ms_per_call"
        route_worker_ms_per_job_mean = Get-G51Mean $rows "route_worker_ms_per_job"
        prefill_vram_seed_gib_mean = Get-G51Mean $rows "prefill_vram_seed_gib"
        prefill_vram_seed_seconds_mean = Get-G51Mean $rows "prefill_vram_seed_seconds"
        runtime_process_read_gib_mean = Get-G51Mean $rows "runtime_process_read_gib"
        runtime_process_write_gib_mean = Get-G51Mean $rows "runtime_process_write_gib"
        runtime_vram_peak_mib_mean = Get-G51Mean $rows "runtime_vram_peak_mib"
        runtime_dedicated_peak_gib_mean = Get-G51Mean $rows "runtime_dedicated_peak_gib"
        runtime_shared_peak_gib_mean = Get-G51Mean $rows "runtime_shared_peak_gib"
        runtime_working_set_peak_gib_mean = Get-G51Mean $rows "runtime_working_set_peak_gib"
...
$summary = [pscustomobject]@{
    schema = "g51_prefill_vram_seed_ab_v1"
    question = "Current SOTA control versus request-scoped prefill VRAM seed of 8 experts per routed layer on the exact G45 cyberpunk prompt."
    prompt = $prompt
    context = 256
    max_tokens = 64
    arena_gib = 30
    ...
    execution_runner_sha256 = if ($SummarizeExisting) {
        ...
        Get-G51SHA256 $MyInvocation.MyCommand.Path
    }
...
$summaryPath = Join-Path $outdir "g51_prefill_vram_seed_ab_result.json"
