...
function Find-G130U1RuntimeMarker {
    ...
    # ds4_cuda.cu:33472,33567 entry-contract; 33598 dynamic-promotion-contract; 33673 cold-one-contract; 33782 route-entry-contract.
    $a7Contract=[regex]::Match($Text,'(?im)\breason=(entry-contract|dynamic-promotion-contract|cold-one-contract|route-entry-contract)\b')
    if($a7Contract.Success){return New-G130U1WatchdogDecision $true 'A-7' 'A7_RESIDENCY_CONTRACT_VIOLATION' $a7Contract.Value}
...
function Test-G130U1WatchdogSample {
    [CmdletBinding()]
    param([object]$Sample,[object]$State,[UInt64]$BaselinePageOut,[UInt64]$PageOutDeltaAbortPages,[int]$WallCap,[int]$StartupStall,[int]$TtftCap,[int]$DataStall,[double]$FloorTps,[int]$WarmTokens,[int]$ConsecutiveTokens)
    ...
    if($pageout-lt$BaselinePageOut){return New-G130U1WatchdogDecision $true 'A-9' 'A9_PAGEOUT_COUNTER_REGRESSED' "baseline=$BaselinePageOut current=$pageout"};if($pageout-$BaselinePageOut-gt$PageOutDeltaAbortPages){return New-G130U1WatchdogDecision $true 'A-1' 'A1_PAGE_OUT_DELTA' "delta=$($pageout-$BaselinePageOut) threshold=$PageOutDeltaAbortPages"}
...
Assert-G130U1ExactPropertySet $manifest.thresholds @('cadence_seconds','wall_cap_seconds','startup_stall_seconds','ttft_cap_seconds','application_data_stall_seconds','page_out_delta_abort_pages','throughput_floor_tps','throughput_warm_tokens','throughput_consecutive_tokens') 'watchdog thresholds'
...
while($true){$sequence++;$elapsed=$clock.Elapsed.TotalSeconds;$probeOk=$true;$probeError='';try{$os=Get-CimInstance Win32_OperatingSystem -ErrorAction Stop;$raw=Get-CimInstance Win32_PerfRawData_PerfOS_Memory -ErrorAction Stop;$available=[UInt64]$os.FreePhysicalMemory*[UInt64]1024;$pageout=[UInt64]$raw.PagesOutputPerSec}catch{$probeOk=$false;$probeError=$_.Exception.Message;$available=[UInt64]0;$pageout=[UInt64]0}
    ...
    if(-not$probeOk){$decision=New-G130U1WatchdogDecision $true 'A-9' 'A9_MONITOR_SOURCE_FAILURE' $probeError}else{$t=$manifest.thresholds;$decision=Test-G130U1WatchdogSample $sample $state ([UInt64]$manifest.page_out_baseline) ([UInt64]$t.page_out_delta_abort_pages) ([int]$t.wall_cap_seconds) ([int]$t.startup_stall_seconds) ([int]$t.ttft_cap_seconds) ([int]$t.application_data_stall_seconds) ([double]$t.throughput_floor_tps) ([int]$t.throughput_warm_tokens) ([int]$t.throughput_consecutive_tokens)}
    ...
    if(Test-Path -LiteralPath ([string]$manifest.monitor_complete_path) -PathType Leaf){
        try {
            ...
            if($stderrResidual.Count){
                ...
                $residualDecision=Test-G130U1WatchdogSample $residualSample $state ([UInt64]$manifest.page_out_baseline) ([UInt64]$t.page_out_delta_abort_pages) ([int]$t.wall_cap_seconds) ([int]$t.startup_stall_seconds) ([int]$t.ttft_cap_seconds) ([int]$t.application_data_stall_seconds) ([double]$t.throughput_floor_tps) ([int]$t.throughput_warm_tokens) ([int]$t.throughput_consecutive_tokens)
