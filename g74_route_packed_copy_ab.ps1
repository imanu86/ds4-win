# G74 route-packed-copy static32 A/B runner (PowerShell 5.1, ASCII).
param(
    [switch]$Resume,
...
function Get-G74ResultPath {
    param([string]$Tag)
    Join-Path $outdir ("g7_" + $Tag + "_result.json")
}

function Get-G74PropertyValue {
...
function Get-G74AnyPropertyValue {
    param([object]$Object, [string[]]$Names, $Default = $null)
    foreach ($name in $Names) {
        $value = Get-G74PropertyValue -Object $Object -Name $name -Default $null
        if ($null -ne $value) { return $value }
    }
    return $Default
}

function Assert-G74StaticContract {
    if (-not (Test-Path -LiteralPath $harness -PathType Leaf)) {
        throw "G74 static check failed: harness missing: $harness"
    }
    $tokens = $null
    $errors = $null
    ...
    if ($errors -and $errors.Count -gt 0) {
        throw ("G74 static check failed: runner AST errors: " +
    ...
    foreach ($needle in @(
        ...
        if ($harnessText -notmatch [regex]::Escape($needle)) {
            throw "G74 static check failed: harness lacks $needle"
        }
    }
}

function Get-G74ArmConfig {
    param([Parameter(Mandatory=$true)][ValidateSet("static32_split_fused","static32_split_fused_route_packed_copy")][string]$Arm)
    [pscustomobject]@{
        arm = $Arm
        split_fused = $true
        route_packed_copy = ($Arm -eq "static32_split_fused_route_packed_copy")
        budget = 32
    }
}

function Test-G74CapacityFailure {
    param([string]$Message, [string]$Tag)
    $resultPath = Get-G74ResultPath $Tag
    if (Test-Path -LiteralPath $resultPath -PathType Leaf) {
        try {
            $result = Get-Content -LiteralPath $resultPath -Raw | ConvertFrom-Json
            $arenaCapped = [bool](Get-G74PropertyValue -Object $result `
                -Name "dynamic_arena_cap_capped" -Default $false)
            $arenaSlots = [int](Get-G74PropertyValue -Object $result `
                -Name "dynamic_arena_allocated_slots" -Default 0)
            $runtimeAborted = [bool](Get-G74PropertyValue `
                -Object $result.runtime_telemetry `
                -Name "contamination_abort_observed" -Default $false)
            $runtimeAvailable = [uint64](Get-G74PropertyValue `
                -Object $result.runtime_telemetry `
...
function Assert-G74SplitFusedContract {
    ...
    if (-not $requested -or -not $observed -or
        ...
        throw "G74 split-fused contract mismatch"
    }
...
function Get-G74PackedCopyCounters {
    param([Parameter(Mandatory=$true)][object]$Result)
    [pscustomobject]@{
        requested = [bool](Get-G74AnyPropertyValue $Result @(
            "packed_copy_requested",
            "route_packed_copy_requested",
            "gpu_resident_routes_packed_copy_requested",
            "gpu_resident_routes_route_packed_copy_requested") $false)
        observed = [bool](Get-G74AnyPropertyValue $Result @(
            "packed_copy_observed",
            "route_packed_copy_observed",
            "gpu_resident_routes_packed_copy_observed",
            "gpu_resident_routes_route_packed_copy_observed") $false)
        experts = [uint64](Get-G74AnyPropertyValue $Result @(
            "packed_copy_experts",
            "route_packed_copy_experts",
            "gpu_resident_routes_packed_copy_experts",
            "gpu_resident_routes_route_packed_copy_experts") 0)
        submissions = [uint64](Get-G74AnyPropertyValue $Result @(
            "packed_copy_submissions",
            "route_packed_copy_submissions",
            "gpu_resident_routes_packed_copy_submissions",
            "gpu_resident_routes_route_packed_copy_submissions") 0)
        bytes = [uint64](Get-G74AnyPropertyValue $Result @(
            "packed_copy_bytes",
            "route_packed_copy_bytes",
            "gpu_resident_routes_packed_copy_bytes",
            "gpu_resident_routes_route_packed_copy_bytes") 0)
        legacy_submissions = [uint64](Get-G74AnyPropertyValue $Result @(
            "legacy_submissions",
            "legacy_copy_submissions",
            "route_legacy_submissions",
            "route_packed_copy_legacy_submissions",
            "gpu_resident_routes_legacy_submissions",
            "gpu_resident_routes_legacy_copy_submissions") 0)
    }
}

function Assert-G74PackedCopyContract {
    param(
        [Parameter(Mandatory=$true)][object]$Result,
        [Parameter(Mandatory=$true)][object]$Split,
        [Parameter(Mandatory=$true)][bool]$Candidate
    )

    $packed = Get-G74PackedCopyCounters -Result $Result
    $routeMissExperts = [uint64]$Split.misses
    $expectedLegacySubmissions = [uint64](3 * $routeMissExperts)
    $tierRamH2DBytes = [uint64]$Result.expert_tiering.ram_h2d_bytes

    if ($Candidate) {
        if (-not $packed.requested -or -not $packed.observed -or
            ($packed.experts -ne 10692 -and
             ($packed.experts -eq 0 -or $packed.experts -ne $routeMissExperts)) -or
            $packed.submissions -ne $packed.experts -or
            $packed.bytes -ne $tierRamH2DBytes -or
            $packed.legacy_submissions -ne 0) {
            throw "G74 packed-copy candidate contract mismatch"
        }
    } else {
        if ($packed.requested -or $packed.observed -or
            $packed.experts -ne 0 -or
            $packed.submissions -ne 0 -or
            $packed.bytes -ne 0 -or
            $packed.legacy_submissions -ne $expectedLegacySubmissions) {
            throw "G74 packed-copy control contract mismatch"
        }
    }

    [pscustomobject]@{
        requested = $packed.requested
        observed = $packed.observed
        experts = $packed.experts
        submissions = $packed.submissions
        bytes = $packed.bytes
        legacy_submissions = $packed.legacy_submissions
        route_miss_experts = $routeMissExperts
        expected_legacy_submissions = $expectedLegacySubmissions
        tier_ram_h2d_bytes = $tierRamH2DBytes
    }
}

function Assert-G74CommonContract {
    param(
        [Parameter(Mandatory=$true)][object]$Result,
        [Parameter(Mandatory=$true)][string]$Tag,
        [Parameter(Mandatory=$true)][ValidateSet("static32_split_fused","static32_split_fused_route_packed_copy")][string]$Arm,
        [Parameter(Mandatory=$true)][bool]$Safety
    )

    $cfg = Get-G74ArmConfig -Arm $Arm
    ...
    if (-not $Result.outputs_identical -or
        ...
        throw "G74 common contract mismatch: tag=$Tag"
    }
    ...
    if (-not $Result.arena_wrap_unlock_source_ranges_requested -or
        ...
        throw "G74 source reclaim contract mismatch: tag=$Tag"
    }

    $split = Assert-G74SplitFusedContract -Result $Result
    $packed = Assert-G74PackedCopyContract `
        -Result $Result -Split $split -Candidate $cfg.route_packed_copy
    ...
    [pscustomobject]@{
        ...
        packed_copy_requested = [bool]$packed.requested
        packed_copy_observed = [bool]$packed.observed
        packed_copy_experts = [uint64]$packed.experts
        packed_copy_submissions = [uint64]$packed.submissions
        packed_copy_bytes = [uint64]$packed.bytes
        legacy_submissions = [uint64]$packed.legacy_submissions
        route_miss_experts = [uint64]$packed.route_miss_experts
        expected_legacy_submissions = [uint64]$packed.expected_legacy_submissions
        tier_ram_h2d_bytes = [uint64]$packed.tier_ram_h2d_bytes
        result_path = Get-G74ResultPath $Tag
        head = $Result.head
        ...
        disk_read_gib = [math]::Round([uint64](Get-G74PropertyValue $Result.runtime_telemetry "aggregate_disk_read_bytes_estimated" 0) / 1GB, 6)
        process_read_gib = [math]::Round([uint64]$Result.runtime_telemetry.win32_process_read_transfer_delta_bytes / 1GB, 6)
    }
}

function Invoke-G74Run {
    param(
        [Parameter(Mandatory=$true)][string]$Tag,
        [Parameter(Mandatory=$true)][ValidateSet("static32_split_fused","static32_split_fused_route_packed_copy")][string]$Arm,
        [Parameter(Mandatory=$true)][bool]$Safety
    ...
    $args += "-SplitFused"
    if ($cfg.route_packed_copy) { $args += "-RoutePackedCopy" }

    try {
        if (($Resume -or $SummarizeExisting) -and
            (Test-Path -LiteralPath $resultPath -PathType Leaf)) {
            Write-Host ("[g74] resume tag=" + $Tag + " arm=" + $Arm)
        } elseif ($SummarizeExisting) {
            throw "SummarizeExisting result missing: tag=$Tag"
        } else {
            Write-Host ("[g74] start tag=" + $Tag + " arm=" + $Arm)
            ...
            if ($LASTEXITCODE -ne 0) {
                ...
                throw "G74 harness failed: tag=$Tag exit=$LASTEXITCODE output=$tail"
            }
        }

        if (-not (Test-Path -LiteralPath $resultPath -PathType Leaf)) {
            throw "G74 result missing: tag=$Tag"
        }
        $result = Get-Content -LiteralPath $resultPath -Raw | ConvertFrom-Json
        return Assert-G74CommonContract -Result $result -Tag $Tag -Arm $Arm -Safety $Safety
    } catch {
        ...
        [pscustomobject]@{
            ...
            safety = $Safety
            failed = $true
            capacity_failure = (Test-G74CapacityFailure -Message $message -Tag $Tag)
            message = $message
...
function Assert-G74SameProvenance {
    ...
    foreach ($field in $provenanceFields) {
        ...
        if ($values.Count -ne 1) { throw "G74 mixed provenance: field=$field" }
    }
}

function Test-G74Outlier {
    param([object[]]$Rows)
    foreach ($property in @(
            "decode_tokens_per_second", "wrap_seconds",
            "ttft_minus_wrap_seconds")) {
        foreach ($arm in @("static32_split_fused", "static32_split_fused_route_packed_copy")) {
...
function New-G74ArmSummary {
    ...
    foreach ($arm in @("static32_split_fused", "static32_split_fused_route_packed_copy")) {
        ...
        $summaries += [pscustomobject]@{
            ...
            decode_tokens_per_second_mean = Get-G74Mean $armRows "decode_tokens_per_second"
            decode_tokens_per_second_median = Get-G74Median $armRows "decode_tokens_per_second"
            decode_seconds_mean = Get-G74Mean $armRows "decode_seconds"
            ttft_mean_seconds = Get-G74Mean $armRows "ttft_seconds"
            wrap_mean_seconds = Get-G74Mean $armRows "wrap_seconds"
            ttft_minus_wrap_mean_seconds = Get-G74Mean $armRows "ttft_minus_wrap_seconds"
            route_calls_mean = Get-G74Mean $armRows "route_calls"
            split_fused_calls_mean = Get-G74Mean $armRows "split_fused_calls"
            split_fused_hits_mean = Get-G74Mean $armRows "split_fused_hits"
            split_fused_misses_mean = Get-G74Mean $armRows "split_fused_misses"
            split_fused_miss_scratch_bytes_avoided_mean = Get-G74Mean $armRows "split_fused_miss_scratch_bytes_avoided"
            split_fused_sum_read_bytes_avoided_mean = Get-G74Mean $armRows "split_fused_sum_read_bytes_avoided"
            packed_copy_experts_mean = Get-G74Mean $armRows "packed_copy_experts"
            packed_copy_submissions_mean = Get-G74Mean $armRows "packed_copy_submissions"
            packed_copy_bytes_mean = Get-G74Mean $armRows "packed_copy_bytes"
            legacy_submissions_mean = Get-G74Mean $armRows "legacy_submissions"
            route_miss_experts_mean = Get-G74Mean $armRows "route_miss_experts"
            tier_ram_h2d_bytes_mean = Get-G74Mean $armRows "tier_ram_h2d_bytes"
            vram_hits_mean = Get-G74Mean $armRows "vram_hits"
            ram_hits_mean = Get-G74Mean $armRows "ram_hits"
            ram_h2d_gib_mean = Get-G74Mean $armRows "ram_h2d_gib"
            policy_replacements_mean = Get-G74Mean $armRows "policy_replacements"
            policy_budget_skips_mean = Get-G74Mean $armRows "policy_budget_skips"
            route_wait_ms_per_call_mean = Get-G74Mean $armRows "route_wait_ms_per_call"
            route_worker_ms_per_job_mean = Get-G74Mean $armRows "route_worker_ms_per_job"
...
function Write-G74Summary {
    ...
    if ($successful.Count -gt 0) { Assert-G74SameProvenance -Rows $successful -RequireHead $false }
    if ($matrix.Count -gt 0) { Assert-G74SameProvenance -Rows $matrix -RequireHead $true }

    $summary = [pscustomobject]@{
        schema = "g74_route_packed_copy_ab_v1"
        status = $Status
        stop_reason = $StopReason
        performance_claim_allowed = ($Status -eq "matrix_complete" -and
            (@($matrix | Where-Object { $_.arm -eq "static32_split_fused" }).Count -ge 3) -and
            (@($matrix | Where-Object { $_.arm -eq "static32_split_fused_route_packed_copy" }).Count -ge 3))
        ...
        arms = @(
            [pscustomobject]@{
                ...
                required_legacy_submissions = "3 * split_fused_misses"
            },
            [pscustomobject]@{
                arm = "static32_split_fused_route_packed_copy"; replacement_budget = 32
                split_fused = $true
                route_packed_copy = $true
                required_split_fused_calls = "route_calls"
                required_hit_miss_accounting = "split_fused_hits + split_fused_misses == expert_tiering.selected"
                required_avoided_counters = "positive"
                required_packed_copy_experts = "10692, or >0 and equal split_fused_misses"
                required_packed_copy_submissions = "packed_copy_experts"
                required_packed_copy_bytes = "expert_tiering.ram_h2d_bytes"
                required_legacy_submissions = 0
            }
        )
        stop_conditions = @(
            "static32_split_fused_route_packed_copy_safety_failure => stop immediately; no matrix",
            ...
            "both arms split_fused_calls must equal route_calls",
            "both arms split_fused_hits plus split_fused_misses must equal selected",
            "both arms split-fused avoided counters must be positive",
            "candidate packed_copy_requested and packed_copy_observed must be true",
            "candidate packed_copy_experts must be 10692, or at minimum >0 and equal split_fused_misses",
            "candidate packed_copy_submissions must equal packed_copy_experts",
            "candidate packed_copy_bytes must equal expert_tiering.ram_h2d_bytes",
            "candidate legacy_submissions must be zero",
            "control packed-copy counters must be off/zero and legacy_submissions must equal 3 * split_fused_misses",
            "never issue a verdict from n=1 safety timing"
        )
        order = @($Rows | ForEach-Object { $_.tag })
        runs = $Rows
        primary_run_tags = @($matrix | ForEach-Object { $_.tag })
        arm_summary = New-G74ArmSummary $matrix
    ...
    Write-Host ("[g74] summary: " + $summaryPath)
}

New-Item -ItemType Directory -Force -Path $outdir | Out-Null
Assert-G74StaticContract
if ($StaticCheckOnly) {
    Write-Host "[g74] static check passed"
    exit 0
}
...
$candidateSafety = Invoke-G74Run -Tag "g74_static32_split_fused_route_packed_copy_safety_exact" -Arm "static32_split_fused_route_packed_copy" -Safety $true
$runs += $candidateSafety
if ($candidateSafety.failed) {
    Write-G74Summary -Rows $runs -Status "stopped" -StopReason "static32_split_fused_route_packed_copy_safety_failed" -OutlierExtensionTriggered $false
    exit 1
}

$matrixPlan = @(
    @("g74_static32_split_fused_a", "static32_split_fused"),
    @("g74_static32_split_fused_route_packed_copy_a", "static32_split_fused_route_packed_copy"),
    @("g74_static32_split_fused_route_packed_copy_b", "static32_split_fused_route_packed_copy"),
    @("g74_static32_split_fused_b", "static32_split_fused"),
    @("g74_static32_split_fused_c", "static32_split_fused"),
    @("g74_static32_split_fused_route_packed_copy_c", "static32_split_fused_route_packed_copy")
)

foreach ($entry in $matrixPlan) {
    $row = Invoke-G74Run -Tag $entry[0] -Arm $entry[1] -Safety $false
    $runs += $row
    if ($row.failed) {
        Write-G74Summary -Rows $runs -Status "invalid" -StopReason ("matrix_run_failed:" + $row.tag) -OutlierExtensionTriggered $outlierExtensionTriggered
        exit 1
    }
}

if (Test-G74Outlier -Rows @($runs | Where-Object { -not $_.failed })) {
    $outlierExtensionTriggered = $true
    $extensionPlan = @(
        @("g74_static32_split_fused_x1", "static32_split_fused"),
        @("g74_static32_split_fused_route_packed_copy_x1", "static32_split_fused_route_packed_copy"),
        @("g74_static32_split_fused_x2", "static32_split_fused"),
        @("g74_static32_split_fused_route_packed_copy_x2", "static32_split_fused_route_packed_copy"),
        @("g74_static32_split_fused_x3", "static32_split_fused"),
        @("g74_static32_split_fused_route_packed_copy_x3", "static32_split_fused_route_packed_copy")
    )
    foreach ($entry in $extensionPlan) {
        $row = Invoke-G74Run -Tag $entry[0] -Arm $entry[1] -Safety $false
        $runs += $row
        if ($row.failed) {
            Write-G74Summary -Rows $runs -Status "invalid" -StopReason ("outlier_extension_run_failed:" + $row.tag) -OutlierExtensionTriggered $outlierExtensionTriggered
            exit 1
        }
    }
}

Write-G74Summary -Rows $runs -Status "matrix_complete" -StopReason "completed_preregistered_sequence" -OutlierExtensionTriggered $outlierExtensionTriggered
